            ...
            if (!this._panelTrigger) {
                setTimeout(() => {
                    ...
                    if (this.enabled) {
                        ...
                        // check if side menu buttons are theyre (more then onelayer)... if so
                        // find the right buttona and trigger the click event from the active one
    ...
    */
    clickDetails(event: any) {
        ...
        if (layerName === event.data.layerName) {
            ...
            subSection[0].prepend(new DOMParser().parseFromString('<div class="rv-chart-details"></div>', 'text/html')
                .body.getElementsByClassName('rv-chart-details')[0]);
            subSection[0].getElementsByClassName('rv-chart-details')[0].appendChild(new DOMParser().parseFromString(event.data.html, 'text/html').body);
        } else {
